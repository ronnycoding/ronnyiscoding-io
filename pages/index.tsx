...
import { Url, ProjectCategory } from '../src/generated/graphql';
...
export default function Index({ url, projectCategories }: IndexProps) {
  const { page } = url;
  const { seo } = page || {};
  ...
  return (
    <Layout seo={seo}>
      <Container>
        ...
        <div className='lg:grid grid-cols-2 pt-6'>
          {page && page?.introSection && (
            <HeaderDescription
              ...
              dangerouslySetInnerHTML={{
                __html:
                  (page.introSection && page.introSection?.briefDescription) ||
                  '',
              }}
            />
          )}
        ...
        {page &&
          page?.imageTexts &&
          page?.imageTexts.map(({ image, title, text, id }) => (
